...
var AssemblyPrograms = []AssemblyProgram{
	{"simple line counter",
		"/$/ {\n" +
			"  inc(\"line-count\")\n" +
			"}\n",
...
func TestAssembly(t *testing.T) {
	for _, tc := range AssemblyPrograms {
		p := NewParser(tc.name, strings.NewReader(tc.program))
		EmtailParse(p)

		if p.root == nil {
			t.Errorf("Errors parsing %s\n%q\n", tc.name, p.errors)
			continue
		}
		a := &asm{}
		p.root.acceptVisitor(a)
		if !reflect.DeepEqual(tc.assembly, a.assembly) {
			t.Errorf("Assembly doesn't match.\n\texpected: %q\n\treceived: %q\n",
				tc.assembly,
				a.assembly)
		}
	}
